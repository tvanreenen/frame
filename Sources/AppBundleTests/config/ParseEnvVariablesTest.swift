...
final class ParseEnvVariablesTest: XCTestCase {
    ...
    func testCyclicDep() {
        ...
        assertEquals(errors.descriptions, [
            "exec.env-vars.BAR: Env variable 'FOO' isn't presented in frame env vars, or not available for interpolation (because it's mutated)",
            "exec.env-vars.FOO: Env variable 'BAR' isn't presented in frame env vars, or not available for interpolation (because it's mutated)",
        ])
    }
