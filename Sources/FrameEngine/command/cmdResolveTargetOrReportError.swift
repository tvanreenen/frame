...
extension CmdArgs {
    ...
    @MainActor
    package func resolveTargetOrReportError(_ env: CmdEnv, _ io: CmdIo) -> LiveFocus? {
        // Flags
        if let windowId {
        ...
        if let windowId = env.windowId {
            if let wi = Window.get(byId: windowId) {
                ...
                io.err("Invalid <window-id> \(windowId) specified in \(FRAME_WINDOW_ID) env variable")
                return nil
            }
        }
...
extension Window {
    @MainActor
    package func toLiveFocusOrReportError(_ io: CmdIo) -> LiveFocus? {
