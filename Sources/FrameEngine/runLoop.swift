...
extension Thread {
    ...
    package func runInLoop<T>(_ body: @Sendable @escaping (RunLoopJob) throws -> T) async throws -> T { // todo try to convert to typed throws
...
package final class RunLoopJob: Sendable, AeroAny {
    ...
    package init() {}
    package var isCancelled: Bool { _isCancelled == 1 }
    package func cancel() {
        while !isCancelled {
    ...
    package static let cancelled: RunLoopJob = RunLoopJob().also { $0.cancel() }
