...
package class TreeNode: Equatable, AeroAny {
    ...
    package var children: [TreeNode] { _children }
    fileprivate final weak var _parent: NonLeafTreeNodeObject? = nil
    package final var parent: NonLeafTreeNodeObject? { _parent }
    private var adaptiveWeight: CGFloat
    private let _mruChildren: MruStack<TreeNode> = MruStack()
    // Usages:
    // - resize with mouse
    package var lastAppliedLayoutVirtualRect: Rect? = nil  // as if inner gaps were always zero
    // Usages:
    // - resize with mouse
    // - drag window with mouse
    // - move-mouse command
    package var lastAppliedLayoutPhysicalRect: Rect? = nil // with real inner gaps
    package final var unboundStacktrace: String? = nil
    package var isBound: Bool { parent != nil } // todo drop, once https://github.com/tvanreenen/frame/issues/1215 is fixed

    @MainActor package init(parent: NonLeafTreeNodeObject, adaptiveWeight: CGFloat, index: Int) {
    ...
    /// See: ``getWeight(_:)``
    package func setWeight(_ targetOrientation: Orientation, _ newValue: CGFloat) {
    ...
    @MainActor
    package func getWeight(_ targetOrientation: Orientation) -> CGFloat {
    ...
    @MainActor
    @discardableResult
    package func bind(to newParent: NonLeafTreeNodeObject, adaptiveWeight: CGFloat, index: Int) -> BindingData? {
    ...
    package func getUserData<T>(key: TreeNodeUserDataKey<T>) -> T? { userData[key.key] as! T? }
    package func putUserData<T>(key: TreeNodeUserDataKey<T>, data: T) {
        userData[key.key] = data
    }
    @discardableResult
    package func cleanUserData<T>(key: TreeNodeUserDataKey<T>) -> T? { userData.removeValue(forKey: key.key) as! T? }
}

package struct TreeNodeUserDataKey<T> {
...
package struct BindingData {
    package let parent: NonLeafTreeNodeObject
    package let adaptiveWeight: CGFloat
    package let index: Int
    package init(parent: NonLeafTreeNodeObject, adaptiveWeight: CGFloat, index: Int) {
        self.parent = parent
        self.adaptiveWeight = adaptiveWeight
        self.index = index
    }
