...
package func assertTrue(_ actual: Bool, file: String = #filePath, line: Int = #line) {
    assertEquals(actual, true, file: file, line: line)
}

// Because assertEquals default messages are unreadable!
package func assertNotEquals<T>(_ actual: T, _ expected: T, file: String = #filePath, line: Int = #line) where T: Equatable {
    if actual == expected {
        failExpectedActual("not \(expected)", actual, file: file, line: line)
    }
}

package func assertNil(_ actual: Any?, file: String = #filePath, line: Int = #line) {
    if let actual {
        failExpectedActual("nil", actual, file: file, line: line)
    }
}

package func assertNotNil(_ actual: Any?, file: String = #filePath, line: Int = #line) {
    if actual == nil {
        failExpectedActual("not nil", "nil", file: file, line: line)
    }
}

package func assertEquals<T>(_ actual: T, _ expected: T, additionalMsg: String? = nil, file: String = #filePath, line: Int = #line) where T: Equatable {
    if actual != expected {
...
package func assertSucc<T>(_ actual: Result<T, some Any>, file: String = #filePath, line: Int = #line) {
    switch actual {
...
package func assertSucc<T>(_ actual: Result<T, some Any>, _ expected: T, file: String = #filePath, line: Int = #line) where T: Equatable {
    switch actual {
        case .failure: failExpectedActual("Result.success", actual, file: file, line: line)
        case .success(let actual): assertEquals(actual, expected, file: file, line: line)
    }
}
package func assertFail<F>(_ actual: Result<some Any, F>, _ expected: F? = nil, file: String = #filePath, line: Int = #line) where F: Equatable {
    switch actual {
...
package func testParseCommandSucc(_ command: String, _ expected: any CmdArgs) {
